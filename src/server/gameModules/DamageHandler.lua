...
function DamageHandler:playerHitEntity(player, instance, direction)
    local store = self.store
    local recsCore = self.recsCore
...
function DamageHandler:onRecsAndStore(recsCore, store)
    ...
    self.store = store
    eAttackActor.OnServerEvent:connect(function(player, hitInfo)

        local instance = hitInfo.target
        local knockbackDirection = hitInfo.direction
        assert(typeof(instance) == "Instance", "Invalid target")

        self:playerHitEntity(player, instance, knockbackDirection)
    end)
end
